        ...
        BEGIN
            IF(NEW.quantity != OLD.quantity) THEN
                IF(NEW.transaction_type = 2) THEN
                    IF(NEW.quantity > (SELECT stock_quantity FROM products WHERE id = NEW.product_id) OR NEW.quantity > (SELECT MIN(current_quantity) FROM transactions WHERE(shipment = NEW.shipment AND product_id = NEW.product_id))) THEN
            ...
            ELSEIF (NEW.transaction_type != OLD.transaction_type) THEN
                if(NEW.transaction_type = 1) THEN
                    IF (SELECT COUNT(*) FROM transactions WHERE shipment = NEW.shipment > 1) THEN
                       SIGNAL SQLSTATE "45000"
                       SET MESSAGE_TEXT = "Error: Shipment was exist.";
                    END IF;
                ELSE 
                    IF NOT EXISTS(SELECT * FROM transactions WHERE transaction_type = 1) THEN
                        SIGNAL SQLSTATE "45000"
                    ...
                    IF NOT EXISTS(SELECT 1 FROM transactions WHERE transaction_type = 1 AND product_id = NEW.product_id AND expiration_date = NEW.expiration_date) THEN
                          SIGNAL SQLSTATE "45000"
                          SET MESSAGE_TEXT = "Error: Expiration date does not exist.";
                      END IF;
                
                    IF NOT EXISTS(SELECT 1 FROM transactions WHERE transaction_type = 1 AND product_id = NEW.product_id AND transaction_date = NEW.transaction_date) THEN
                       SIGNAL SQLSTATE "45000"
              ...
              ELSEIF(NEW.product_id != OLD.product_id) THEN
                IF NOT EXISTS(SELECT 1 FROM products WHERE id = NEW.product_id) THEN
                  ...
                  IF(NEW.transaction_type = 2) THEN
                      IF NOT EXISTS(SELECT shipment FROM transactions WHERE(transaction_type = 1 AND product_id = NEW.product_id AND shipment = NEW.shipment)) THEN
                          SIGNAL SQLSTATE "45000"
